...
Page({
      ...
      http('/api-member/customerAddress/'+id, null, null, 'get').then(res => {
        if (res.code == '100000') {
          that.setData({
            ...
            selProvince: res.data.obj.province,
            selCity: res.data.obj.city,
            selDistrict: res.data.obj.region
          });
          that.setDBSaveAddressId(res.data.obj);
          wx.hideLoading();
        } else {
  ...
  switchChange: function (e) {
    ...
    //   type: 'default'
    // }
    // http('/api-member/customerAddress/updateAddress/' + this.data.id, param, null, 'post').then(res => {
    //   if (res.code == '100000') {
    //   } else {
